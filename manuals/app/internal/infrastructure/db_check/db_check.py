...
def check_section_content(id):
    ...
    page_element = PageElement(id=id, content= { "items" : column_ids }, type=PageElement.ElementType.COLUMN_LIST)
    page_element.save()
...
def check_column_list_content(id):
    ...
    page_element = PageElement(id=id, content= { "items" : element_ids }, type=PageElement.ElementType.COLUMN_LIST)
    page_element.save()
...
def check_column_content(id):
    ...
    page_element = PageElement(id=id, content= { "items" : item_ids }, type=PageElement.ElementType.COLUMN)
    page_element.save()

    for item in content["results"]:
        if item["type"] == "paragraph":
            page_element = PageElement(id=item["id"], content= { "paragraph" : [ {"text" : item["paragraph"]["text"][0]["text"] }, {"annotations" : item["paragraph"]["text"][0]["annotations"] }]}, type=PageElement.ElementType.PARAGRAPH)
            page_element.save()
        if item["type"] == "image":
            image_id = item["id"]
            ...
            page_element = PageElement(id=image_id, content= { "image" : image_name }, type=PageElement.ElementType.IMAGE)
            page_element.save()