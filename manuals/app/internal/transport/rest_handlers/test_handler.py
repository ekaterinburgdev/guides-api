...
notion = NotionClient()
notion_client = notion.notion_client
...
class ChildrenView(View):
    def get(self, request):
        if request.GET.getlist("id"):
            cursor = None
            if request.GET.getlist("cursor"):
                cursor = request.GET.getlist("cursor")[0]
            id = request.GET.getlist("id")[0]
            if cursor:
                page_content = notion.page_children_from_cursor(id, cursor)
            else:
